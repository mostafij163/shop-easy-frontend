...
export default function CreateNewShop() {
  ...
  const [closingHour, setClosingingHour] = useState(
    new Date().toLocaleTimeString()
  );
    ...
    if (userJwt) {
      ...
      try {
        res = await axios.post(
          "http://127.0.0.1:8000/shop/new-shop",
          {
            name: title,
        ...
        <div>
          ...
          >
            <DialogTitle id="alert-dialog-title">
              {"Select Your Shop's Exact Location"}
            </DialogTitle>
            <DialogContent>
              <div>
                <ReactMapGL
        ...
        <form onSubmit={newShopFormSubmit}>
          ...
          <div className={newShopStyles["btn-container"]}>
            <Button
              variant="contained"
              type="submit"
              className={newShopStyles["submit-btn"]}
              color="primary"
